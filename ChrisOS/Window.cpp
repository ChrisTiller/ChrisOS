...
void Window::events(sf::Event &event, sf::RenderWindow &window)
{
	...
	case sf::Event::MouseButtonPressed:
		if (mMouseOver) {
			...
			{
				mWindowMsgQueue.addMessage(mWindowID, LEFT_MOUSE_BUTTON_DOWN, event);
				mButtonPress = true;
			}
		}
		mWindowMsgQueue.addMessage(mWindowID, MOUSE_DRAG, event);
		break;
	case sf::Event::MouseButtonReleased:
		if (mButtonPress) {
			mWindowMsgQueue.addMessage(mWindowID, LEFT_MOUSE_BUTTON_UP, event);
			if (mMouseOver) {
				if (event.mouseButton.button == sf::Mouse::Left)
				{
					if (mWindowID != 0)
	...
	case sf::Event::MouseMoved:
		if (mMouseOver)
		{
			if (!mMouseEnter)
			{
				mWindowMsgQueue.addMessage(mWindowID, MOUSE_ENTER, event);
				mMouseEnter = true;
				mMouseLeave = false;
			}
			if (mButtonPress)
			...
			mWindowMsgQueue.addMessage(mWindowID, MOUSE_MOVE, event);
		}
		else
		{
			if (!mMouseLeave)
			{
				mWindowMsgQueue.addMessage(mWindowID, MOUSE_LEAVE, event);
				mMouseLeave = true;
				mMouseEnter = false;
			}
			if (mButtonPress)
	...
	mEventHandlerClass->messageCallBack(this, mWindowMsgQueue);
}