...
void System::loadSystemFunctions() {
    ...
    systemFunctions.insert(std::make_pair("createPCB", &System::createPCB));
    systemFunctions.insert(std::make_pair("deletePCB", &System::deletePCB));
    systemFunctions.insert(std::make_pair("block", &System::block));
    systemFunctions.insert(std::make_pair("unblock", &System::unblock));
...
void System::createPCB(const vector<string>& params) {
    ...
    if (params.size() == 0) {
        IO::println("createPCB takes 3 parameters");
        return;
    }

    if (params.at(0) == "help") {
        IO::println("\tCommand: createPCB <process name> <process class> <process priority>");
        IO::println("\tParameters:");
        IO::println("\t\t<process name> - the name of the process");
        IO::println("\t\t<process class> - must be either APPLICATION or SYSTEM");
        IO::println("\t\t<process priority> - the priority of the process between -127 and 128");
        //IO::println("\tReturn: returns both the date and the time with the specified format.");
        return;
    }
    ...
    if (findPCB(params.at(0)) != NULL) {
        IO::println("A process with that name already exists");
        return;
    }

    if ((params.at(1) != "APPLICATION") && (params.at(1) != "SYSTEM")) {
        IO::println("Invalid process class");
        return;
    }

    if ((atoi(params.at(2).c_str()) < -127) || ((atoi(params.at(2).c_str()) > 128))) {
        IO::println("Invalid process priority");
        return;
    }

    PCB* pcb;

    if (params.at(1) == "APPLICATION") {
        pcb = setupPCB(params.at(0), atoi(params.at(2).c_str()), APPLICATION);
    } else {
        pcb = setupPCB(params.at(0), atoi(params.at(2).c_str()), SYSTEM);
    }

    insertPCB(pcb);

    IO::println("Process sucessfully created");
...
void System::deletePCB(const vector<string>& params) {
    ...
    if (params.size() == 0) {
        IO::println("deletePCB takes 1 parameter");
        return;
    }

    if (params.at(0) == "help") {
        IO::println("\tCommand: deletePCB <process name>");
        IO::println("\tParameters:");
        IO::println("\t\t<process name> - the name of the process");
        //IO::println("\tReturn: returns both the date and the time with the specified format.");
        return;
    ...
    if (params.size() != 1) {
        IO::println("deletePCB takes 1 parameter1");
        return;
    }
    ...
    if (pcb == NULL) {
        IO::println("A process with that name does not exist");
        return;
    }
    ...
    if (removePCB(pcb)) {
        ...
        IO::println("Process sucessfully removed");
        return;
    }

    IO::println("Process removal failed");
...
void System::block(const vector<string>& params) {
     ...
     if (params.size() == 0) {
        IO::println("block takes 1 parameter");
        return;
    }

    if (params.at(0) == "help") {
        IO::println("\tCommand: block <process name>");
        IO::println("\tParameters:");
        IO::println("\t\t<process name> - the name of the process");
        //IO::println("\tReturn: returns both the date and the time with the specified format.");
        return;
    ...
    if (params.size() != 1) {
        IO::println("block takes 1 parameter1");
        return;
    }
    ...
    if (!removePCB(pcb)) {
        IO::println("Process could not be blocked");
        return;
    }
    ...
    IO::println("Process sucesfully blocked");
...
void System::unblock(const vector<string>& params) {
     ...
     if (params.size() == 0) {
        IO::println("unblock takes 1 parameter");
        return;
    }

    if (params.at(0) == "help") {
        IO::println("\tCommand: unblock <process name>");
        IO::println("\tParameters:");
        IO::println("\t\t<process name> - the name of the process");
        //IO::println("\tReturn: returns both the date and the time with the specified format.");
        return;
    ...
    if (params.size() != 1) {
        IO::println("unblock takes 1 parameter1");
        return;
    }
    ...
    IO::println("Process sucesfully unblocked");
...
void System::showAll(const vector<string>& params) {
    ...
    if (mReadyQueue.numPCB() > 0) {
        ...
        PCB* pcb = mReadyQueue.front();

        while (pcb != mReadyQueue.cend()) {
            displayPCBInfo(pcb);
            pcb = pcb->getNext();
    ...
    if (mBlockedQueue.numPCB() > 0) {
        ...
        PCB* pcb = mBlockedQueue.front();

        while (pcb != mBlockedQueue.cend()) {
            displayPCBInfo(pcb);
            pcb = pcb->getNext();
